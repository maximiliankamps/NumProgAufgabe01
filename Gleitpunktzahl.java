...
public class Gleitpunktzahl {
	...
	public static void denormalisiere(Gleitpunktzahl a, Gleitpunktzahl b) {
		if (a.compareAbsTo(b) >= 1) {
			int shiftNum = a.exponent - b.exponent;
			a.mantisse = a.mantisse << shiftNum;
			a.exponent = a.exponent - shiftNum;
		} else if (a.compareAbsTo(b) <= -1) {
			int shiftNum = b.exponent - a.exponent;
			b.mantisse = b.mantisse << shiftNum;
			b.exponent = b.exponent - shiftNum;
		}
	}
...
//
	...
	public Gleitpunktzahl sub(Gleitpunktzahl r) {
		...
		if (this.mantisse > r.mantisse && !r.vorzeichen && !this.vorzeichen) {
			result.vorzeichen = false;
			result.mantisse = this.mantisse - r.mantisse;

		} else if (this.mantisse < r.mantisse && !this.vorzeichen && !r.vorzeichen) {
			...
			result.vorzeichen = true;
			result.mantisse = r.mantisse + this.mantisse;
		} else if (this.vorzeichen && r.vorzeichen && this.mantisse > r.mantisse) {
			result.vorzeichen = true;
			result.mantisse = this.mantisse - r.mantisse;
		} else if (this.vorzeichen && r.vorzeichen && this.mantisse < r.mantisse) {
			result.vorzeichen = false;
